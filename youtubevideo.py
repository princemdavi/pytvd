...
class YoutubeVideo():
    ...
    def get_streams(self):
        vstreams = jsonable_encoder(self.video.streams.filter(subtype='mp4', only_video=True).order_by(
            "resolution").desc())["fmt_streams"]  # video streams with no audio

        astream = jsonable_encoder(
            self.video.streams.get_audio_only())  # best audio stream

        fvstreams = []  # filtered video streams
        mastream = {"itag": astream["itag"], "size": self.convert_bytes(
            astream["_filesize"])}  # modified audio stream
        ...
        for vstream in vstreams:
            ...
            if not vstream["resolution"] or vstream["resolution"] in resolutions or not vstream["_filesize"] or "avc1." not in vstream["codecs"][0]:
                continue

            fvstreams.append({"itag": vstream["itag"], "res": vstream["resolution"], "size": self.convert_bytes(
                vstream["_filesize"] + astream["_filesize"])})
    ...
    def download(self, itag):
        ...
        if not stream.video_codec:
            ...
            audio_path = stream.download(
                output_path="downloads/", filename=filename)
            ...
            video_path = stream.download(
                output_path="downloads/", filename=video_filename)
            audio_path = audio_stream.download(
                output_path="downloads/", filename=audio_filename)

            input_video = ffmpeg.input(video_path).video
            input_audio = ffmpeg.input(audio_path).audio

            ffmpeg.output(input_video, input_audio, audio_video_path,
    ...
    def streaming_data(self):
        return self.video.streaming_data['formats']
